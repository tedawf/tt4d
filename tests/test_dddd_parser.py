...
def test_historical_draw_accepts_variable_counts_when_not_strict():
    ...
    assert validate_parsed_draw(parsed, strict=False) == []


def test_historical_draw_fails_strict_count_validation():
    parsed = parse_dddd_html(_read_sample("4d_505.html"), requested_draw_number=505)

    errors = validate_parsed_draw(parsed, strict=True)
    assert "starter_count_expected_10_got_8" in errors
    assert "consolation_count_expected_10_got_10" not in errors


def test_live_draw_with_full_23_passes_strict_validation():
    ...
    assert validate_parsed_draw(parsed, strict=True) == []
