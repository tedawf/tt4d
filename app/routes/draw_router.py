...
async def get_draws(
    ...
    for result in results:
        ...
        draw_results.append(_get_draw_result_extra(result, shares))

    return draw_results


@router.get("/latest", response_model=DrawResultSchema)
async def get_latest_draw(db: Session = Depends(get_db)):
    ...
    return _get_draw_result_extra(result, shares)


@router.get("/search")  # must be above dynamic route /{draw_number}
...
@router.get("/{draw_number}", response_model=DrawDetailsSchema)
async def get_draw(draw_number: int, db: Session = Depends(get_db)):
    ...
    return DrawDetailsSchema(
        draw_result=_get_draw_result_extra(result, shares),
